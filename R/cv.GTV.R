...
cv.GTV<-function(X,y,Sigma,family='Guassian',nlambda=100,nfolds=5,metric=c("mclr","l2"),run='all',file_identifier='',i=NULL,l=NULL){
  ...
  if (run == 'stage1') {
    ...
    v.min = c(lam_TV[best.ind.min[1]],lam_ridge[best.ind.min[2]],lam_1TV.min[best.ind.min[1],best.ind.min[2]])
    v.1se = c(lam_TV[best.ind.1se[1]],lam_ridge[best.ind.1se[2]],lam_1TV.1se[best.ind.1se[1],best.ind.1se[2]])
    names(v.min) = c("lam_TV","lam_ridge","lam_1TV")
    names(v.1se) = names(v.min)
    
    # Fit a full model
    fit1 = GTV_v2(X = X,y = y,Sigma = Sigma,lam_TV = v.min[1],lam_ridge = v.min[2],lam_1TV = v.min[3],family = family,Bt=Bt)
    fit2 = GTV_v2(X = X,y = y,Sigma = Sigma,lam_TV = v.1se[1],lam_ridge = v.1se[2],lam_1TV = v.1se[3],family = family,Bt=Bt)
    ...
    lambdas = list(lam_TV=lam_TV,lam_ridge = lam_ridge, lam_1TV.min = lam_1TV.min, lam_1TV.1se = lam_1TV.1se)
